...
class Cfg:
    ...
    notebook_run_require_job_id: bool
    notebook_run_enforce_success: bool
    notebook_expected_exit_value: str
    notebook_run_execution_data: Optional[dict[str, Any]]
    deploy_foundry_models: bool
    foundry_chat_deployment_name: str
    foundry_chat_model_name: str
    foundry_chat_model_version: str
    foundry_chat_sku_name: str
    foundry_chat_capacity: int
    foundry_embedding_deployment_name: str
    foundry_embedding_model_name: str
    foundry_embedding_model_version: str
    foundry_embedding_sku_name: str
    foundry_embedding_capacity: int
    automate_foundry_remaining: bool
...
FAILURE_STATES = {"failed", "error", "cancelled", "canceled", "timedout", "timeout", "deduped"}


def _failure_events_path() -> Path:
    return Path(__file__).resolve().parents[2] / "logs" / "postprovision_failure_events.jsonl"


def record_failure_event(stage: str, code: str, message: str, **context: Any) -> None:
    event = {
    ...
    try:
        path = _failure_events_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(event, sort_keys=True) + "\n")
        print(f"[TRACK] Failure event logged: stage={stage} code={code}")
    except Exception as ex:
        print(f"[WARN] Could not write failure event log: {ex}")
...
def maybe_generate_foundry_support_bundle() -> None:
    if os.getenv("FOUNDRY_SUPPORT_BUNDLE_ON_FAILURE", "true").strip().lower() != "true":
        return

    command = ["python3", "scripts/automation/create_foundry_support_bundle.py"]
    environment = os.getenv("AZURE_ENV_NAME", "").strip()
    if environment:
        command.extend(["--environment", environment])
    output_dir = os.getenv("FOUNDRY_SUPPORT_BUNDLE_DIR", "").strip()
    if output_dir:
        command.extend(["--output-dir", output_dir])
    try:
        run_with_wait_output(
            command,
            label="Creating redacted Foundry support bundle",
            heartbeat_seconds=10,
            check=False,
        )
    except Exception as ex:
        print(f"[WARN] Could not create Foundry support bundle: {ex}")
...
def _discover_fabric_capacity_arm_id(resource_group: str) -> str:
    if not resource_group:
        return ""

    desired_name = os.getenv("FABRIC_CAPACITY_NAME", "").strip().lower()
    try:
        raw = run(
            [
                "az",
        ...
        print(f"[WARN] Could not discover Fabric capacity from Azure resources: {ex}")
        return ""
    ...
    if desired_name:
        for item in items:
            if str(item.get("name", "")).strip().lower() == desired_name:
...
def load_cfg() -> Cfg:
    ...
    if not fabric_capacity_id:
        fabric_capacity_id = _discover_fabric_capacity_arm_id(resource_group)
        if fabric_capacity_id:
            os.environ["FABRIC_CAPACITY_ID"] = fabric_capacity_id
            print(f"[INFO] Discovered FABRIC_CAPACITY_ID from resource group: {fabric_capacity_id}")
    ...
    notebook_run_require_job_id = (
        os.getenv("NOTEBOOK_RUN_REQUIRE_JOB_ID", "true").strip().lower() == "true"
    )
    notebook_run_enforce_success = (
        os.getenv("NOTEBOOK_RUN_ENFORCE_SUCCESS", "true").strip().lower() == "true"
    )
    notebook_expected_exit_value = os.getenv("NOTEBOOK_RUN_EXPECTED_EXIT_VALUE", "").strip()
    notebook_run_execution_data_raw = os.getenv("FABRIC_NOTEBOOK_RUN_EXECUTION_DATA", "").strip()
    notebook_run_execution_data: Optional[dict[str, Any]] = None
    if notebook_run_execution_data_raw:
        try:
            parsed_execution_data = json.loads(notebook_run_execution_data_raw)
        except json.JSONDecodeError as ex:
            raise ValueError(
                "FABRIC_NOTEBOOK_RUN_EXECUTION_DATA must be valid JSON object text"
            ) from ex
        if not isinstance(parsed_execution_data, dict):
            raise ValueError(
                "FABRIC_NOTEBOOK_RUN_EXECUTION_DATA must be a JSON object"
            )
        notebook_run_execution_data = parsed_execution_data
    deploy_foundry_models = os.getenv("DEPLOY_FOUNDRY_MODELS", "true").strip().lower() == "true"
    foundry_chat_deployment_name = os.getenv("FOUNDRY_CHAT_DEPLOYMENT_NAME", "gpt-5.4").strip()
    foundry_chat_model_name = os.getenv("FOUNDRY_CHAT_MODEL_NAME", "gpt-5.4").strip()
    foundry_chat_model_version = os.getenv("FOUNDRY_CHAT_MODEL_VERSION", "2026-03-05").strip()
    foundry_chat_sku_name = os.getenv("FOUNDRY_CHAT_SKU_NAME", "GlobalStandard").strip()
    foundry_chat_capacity = max(1, int(os.getenv("FOUNDRY_CHAT_CAPACITY", "100")))
    ...
    foundry_embedding_sku_name = os.getenv(
        "FOUNDRY_EMBEDDING_SKU_NAME", "Standard"
    ).strip()
    foundry_embedding_capacity = max(1, int(os.getenv("FOUNDRY_EMBEDDING_CAPACITY", "120")))
    automate_foundry_remaining = (
        os.getenv("AUTOMATE_FOUNDRY_REMAINING", "true").strip().lower() == "true"
    )
    ...
    return Cfg(
        ...
        notebook_run_require_job_id=notebook_run_require_job_id,
        notebook_run_enforce_success=notebook_run_enforce_success,
        notebook_expected_exit_value=notebook_expected_exit_value,
        notebook_run_execution_data=notebook_run_execution_data,
        ...
        foundry_chat_sku_name=foundry_chat_sku_name,
        foundry_chat_capacity=foundry_chat_capacity,
        ...
        foundry_embedding_sku_name=foundry_embedding_sku_name,
        foundry_embedding_capacity=foundry_embedding_capacity,
        automate_foundry_remaining=automate_foundry_remaining,
    )
...
def foundry_project_ensure(cfg: Cfg) -> None:
    ...
    if not _foundry_project_location_supported(cfg.location):
        ...
        record_failure_event(
            stage="foundry_project_ensure",
            code="FoundryProjectRegionUnsupported",
            message="Foundry project ARM resource type unavailable for region; manual project creation required.",
            region=cfg.location,
    ...
    if last_resp is not None:
        ...
        record_failure_event(
            stage="foundry_project_ensure",
            code="FoundryProjectArmCreateFailed",
            message="Unable to create Foundry project via ARM API.",
            httpStatus=last_resp.status_code,
            responseSnippet=last_resp.text[:500],
            projectName=cfg.project_name,
            hubName=cfg.hub_name,
            region=cfg.location,
        )
    ...
    try:
        run(
            [
                ...
                f"properties.customSubDomainName={cfg.hub_name}",
            ],
            check=True,
        )
        print("[OK] Hub project management and custom subdomain enabled")
    except Exception as ex:
        print("[WARN] Could not enable hub project management/custom subdomain automatically.")
        print(f"       {ex}")
...
def ensure_foundry_model_deployments(cfg: Cfg) -> None:
    ...
    desired = [
        (
            ...
            cfg.foundry_chat_sku_name,
            cfg.foundry_chat_capacity,
        ),
        (
            cfg.foundry_embedding_deployment_name,
            cfg.foundry_embedding_model_name,
            cfg.foundry_embedding_model_version,
            cfg.foundry_embedding_sku_name,
            cfg.foundry_embedding_capacity,
        ),
    ]

    for deployment_name, model_name, model_version, sku_name, desired_capacity in desired:
        current = existing_by_name.get(deployment_name)
        current_capacity = int((current or {}).get("sku", {}).get("capacity") or 0)
        current_sku = str((current or {}).get("sku", {}).get("name") or "")
        if (
            current is not None
            and current_capacity >= desired_capacity
            and current_sku.lower() == sku_name.lower()
        ):
            print(
                f"[OK] Foundry model deployment exists: {deployment_name} "
                f"(capacity={current_capacity})"
            )
            continue

        try:
            action = "Scaling" if current is not None else "Deploying"
            run_with_wait_output(
                [
                    "az",
                    "cognitiveservices",
                ...
                label=f"{action} Foundry model '{deployment_name}'",
                check=True,
            )
            print(
                f"[OK] Foundry model deployment ready: {deployment_name} "
                f"(capacity={desired_capacity})"
            )
        except Exception as ex:
...
def _discover_resource_identity(
    command: list[str],
) -> tuple[str, str]:
    try:
        raw = run([*command, "-o", "json"], check=True)
        payload = json.loads(raw) if raw else {}
        resource_id = str(payload.get("id") or "").strip()
        principal_id = str((payload.get("identity") or {}).get("principalId") or "").strip()
        return resource_id, principal_id
    except Exception:
        return "", ""


def _discover_foundry_project_principal(cfg: Cfg) -> str:
    try:
        hub_id = run(
            [
                "az",
        ...
        raw = run(
            [
                ...
                "get",
                "--uri",
                f"https://management.azure.com{hub_id}/projects/{cfg.project_name}"
        ...
        payload = json.loads(raw) if raw else {}
        return str((payload.get("identity") or {}).get("principalId") or "").strip()
    except Exception:
        return ""


def ensure_azure_role_assignments(cfg: Cfg) -> None:
    search_scope = os.getenv("SEARCH_SERVICE_ID", "").strip()
    search_principal_id = os.getenv("SEARCH_SERVICE_PRINCIPAL_ID", "").strip()
    if not search_scope or not search_principal_id:
        discovered_scope, discovered_principal = _discover_resource_identity(
            [
                "az",
                "search",
                "service",
                ...
                cfg.resource_group,
                "-n",
                os.getenv("SEARCH_SERVICE_NAME", "").strip(),
            ]
        )
        search_scope = search_scope or discovered_scope
        search_principal_id = search_principal_id or discovered_principal

    ai_services_scope = os.getenv("AI_SERVICES_ID", "").strip()
    if not ai_services_scope:
        ai_services_scope, _ = _discover_resource_identity(
            [
                "az",
                "cognitiveservices",
                "account",
    ...
    if search_scope:
        os.environ["SEARCH_SERVICE_ID"] = search_scope
    if search_principal_id:
        os.environ["SEARCH_SERVICE_PRINCIPAL_ID"] = search_principal_id

    user_principal_id = _signed_in_user_object_id()
    project_principal_id = _discover_foundry_project_principal(cfg)
    ...
    if search_scope and project_principal_id:
        for role_name in (
            "Search Index Data Reader",
            "Search Service Contributor",
        ):
...
def _parse_retry_after_seconds(value: Optional[str], fallback: Optional[int] = None) -> Optional[int]:
    if value is None:
        return fallback
    raw = str(value).strip()
    if not raw:
    ...
    if parsed <= 0:
        return fallback
    # Keep a sane bound to avoid accidental huge sleeps from malformed gateways.
    return min(parsed, 300)


_FABRIC_LAUNCHER_PIP_INSTALL = (
    "%pip install fabric-launcher --quiet --no-warn-conflicts "
    "--disable-pip-version-check"
)

_FABRIC_LAUNCHER_RETRY_INSTALL = """import importlib.util
import subprocess
import sys
import time

def _ensure_fabric_launcher(max_attempts=3):
    if importlib.util.find_spec("fabric_launcher") is not None:
        return
    ...
    raise RuntimeError(
        "Unable to install fabric-launcher in this session. "
        "Check outbound package access/environment policy."
    ) from last_err

_ensure_fabric_launcher()"""


def _harden_launcher_notebook(raw: dict[str, Any]) -> bool:
    hardened = False
    for cell in raw.get("cells", []):
        ...
        if _FABRIC_LAUNCHER_PIP_INSTALL not in content:
            continue
        content = content.replace(
            _FABRIC_LAUNCHER_PIP_INSTALL,
            _FABRIC_LAUNCHER_RETRY_INSTALL,
            1,
        )
...
def _convert_ipynb_to_fabric_py(
    ipynb_path: Path,
...
def try_import_launcher_notebook(
    ...
    launcher_candidates = [
        repo_root / "fabric-main" / "Healthcare_Launcher.ipynb",
    ]
    launcher_path = next((p for p in launcher_candidates if p.exists()), None)
    if launcher_path is None:
        searched = ", ".join(str(p) for p in launcher_candidates)
        print(f"[WARN] Launcher notebook file not found on disk. Searched: {searched}")
        return False, False, None

    print(f"[INFO] Using launcher notebook from disk: {launcher_path}")
    raw_notebook = json.loads(launcher_path.read_text(encoding="utf-8-sig"))
    if _harden_launcher_notebook(raw_notebook):
        print("[INFO] Applied retrying fabric-launcher installation for automation")
    notebook_bytes = json.dumps(raw_notebook, ensure_ascii=False).encode("utf-8")
    ...
    notebook_py = _convert_ipynb_to_fabric_py(launcher_path, raw_notebook)
...
def run_launcher_notebook_if_present(
    cfg: Cfg,
    workspace_id: str,
    ...
    # Prefer the current RunNotebook path and a true no-body request first.
    # Some tenants treat JSON {} differently from an empty request, and the
    # path-based job type is the current documented shape.
    run_paths = [
        ...
        f"workspaces/{workspace_id}/notebooks/{notebook_id}/jobs/RunNotebook/instances",
        f"workspaces/{workspace_id}/notebooks/{notebook_id}/jobs/instances?jobType=RunNotebook",
    ]

    attempts: list[tuple[str, str, Optional[dict[str, Any]]]] = []
    if cfg.notebook_run_execution_data:
        execution_payload = {"executionData": cfg.notebook_run_execution_data}
        print("[INFO] Notebook run will include executionData from FABRIC_NOTEBOOK_RUN_EXECUTION_DATA")
        for path in run_paths:
            attempts.append(("POST", path, execution_payload))

    for path in run_paths:
        attempts.append(("POST", path, None))

    # Last fallback for tenants that expect an explicit empty JSON object.
    for path in run_paths:
        attempts.append(("POST", path, {}))
    ...
    for method, path, body in attempts:
        ...
        if r.status_code in (200, 201, 202):
            ...
            if job_id:
                ...
                return notebook_id, job_id, retry_after
            ...
            print("[WARN] Unable to deterministically track run state because job id was not returned.")
            print("       This can happen in some tenants/gateways when run-start metadata is partially omitted.")
            return notebook_id, None, retry_after
...
def get_job_status(
    ...
    for path in paths:
        ...
        if r.status_code == 200:
            retry_after = _parse_retry_after_seconds(r.headers.get("Retry-After"))
            return (r.json() if r.content else {}), retry_after
    return None, None
...
def wait_for_notebook_completion(
    ...
    notebook_id: str,
    job_id: str,
    initial_poll_seconds: Optional[int] = None,
) -> Optional[str]:
    token = get_token("https://api.fabric.microsoft.com")
    max_seconds = cfg.notebook_max_minutes * 60
    base_interval = max(cfg.notebook_poll_seconds, 10)
    next_poll_seconds = max(initial_poll_seconds or base_interval, 5)

    print("[INFO] Waiting for notebook completion...")
    print(f"       Base poll {base_interval}s, timeout {cfg.notebook_max_minutes} minutes")
    ...
    while True:
        ...
        status_payload, retry_after = get_job_status(token, workspace_id, notebook_id, job_id)
        ...
        if status_payload is None:
            ...
            if retry_after:
                next_poll_seconds = max(retry_after, 5)
            else:
                next_poll_seconds = base_interval
            print(
                f"[STATUS] Working on {cfg.notebook_name} | running for {elapsed_minutes} minute(s)"
            ...
            if status in TERMINAL_STATES:
                if status in SUCCESS_STATES:
                    exit_value = status_payload.get("exitValue")
                    if exit_value is not None:
                        print(f"[INFO] Notebook exitValue: {exit_value}")
                    if cfg.notebook_expected_exit_value:
                        expected = cfg.notebook_expected_exit_value
                        actual = "" if exit_value is None else str(exit_value)
                        if actual != expected:
                            print(
                                "[ERROR] Notebook exitValue did not match expected value."
                            )
                            print(f"       expected: {expected}")
                            print(f"       actual:   {actual or '<missing>'}")
                            return "failed_exitvalue"
...
def main() -> int:
    ...
    print(f"Require job ID: {cfg.notebook_run_require_job_id}")
    print(f"Enforce success: {cfg.notebook_run_enforce_success}")
    if cfg.notebook_expected_exit_value:
        print(f"Expected exitValue: {cfg.notebook_expected_exit_value}")
    if cfg.notebook_run_execution_data:
        print("Notebook run payload: executionData enabled")
    print(f"Automate remaining Foundry steps: {cfg.automate_foundry_remaining}")
    if cfg.turbo_deploy:
    ...
    ensure_azure_role_assignments(cfg)
    print("[STEP] Ensuring Fabric workspace")
    ws_id = fabric_workspace_ensure(cfg)
    notebook_run_failed = False
    notebook_run_attempted = False
    notebook_run_final_status: Optional[str] = None
    notebook_run_job_id: Optional[str] = None
    if ws_id:
        ...
        if not skipped_launcher:
            notebook_id, job_id, initial_retry_after = run_launcher_notebook_if_present(cfg, ws_id)
            notebook_run_attempted = notebook_id is not None
            if notebook_id and job_id:
                final_status = None
                attempt_no = 1
                current_notebook_id = notebook_id
                current_job_id = job_id
                current_initial_retry_after = initial_retry_after
                notebook_run_job_id = job_id
                ...
                while current_notebook_id and current_job_id and attempt_no <= cfg.notebook_run_max_attempts:
                    ...
                    final_status = wait_for_notebook_completion(
                        ...
                        current_notebook_id,
                        current_job_id,
                        initial_poll_seconds=current_initial_retry_after,
                    )
                    if final_status:
                        print(
                    ...
                    if final_status in SUCCESS_STATES:
                        break

                    if attempt_no >= cfg.notebook_run_max_attempts:
                        break

                    retry_notebook_id, retry_job_id, retry_after = run_launcher_notebook_if_present(cfg, ws_id)
                    if retry_notebook_id and retry_job_id:
                        current_notebook_id = retry_notebook_id
                        current_job_id = retry_job_id
                        current_initial_retry_after = retry_after
                        notebook_run_job_id = retry_job_id
                        attempt_no += 1
                        continue
                ...
                notebook_run_final_status = final_status
                if final_status not in SUCCESS_STATES:
                    notebook_run_failed = True
            ...
            elif notebook_id and cfg.notebook_run_require_job_id:
                notebook_run_failed = True
                notebook_run_final_status = "indeterminate_missing_job_id"
                print("[ERROR] Notebook run is indeterminate: API accepted run but no job id was available.")
                print("       Failing this run to avoid guessing completion state.")
                print("       You can disable strict behavior with NOTEBOOK_RUN_REQUIRE_JOB_ID=false.")
                record_failure_event(
                    stage="launcher_notebook_run",
                    code="IndeterminateMissingJobId",
                    message="Run submission accepted but no job id was available for deterministic status tracking.",
                    notebookName=cfg.notebook_name,
                    workspaceId=ws_id,
                )
    ...
    if notebook_run_attempted:
        print("[SUMMARY] Notebook run orchestration")
        print(f"         final_status={notebook_run_final_status or '<unknown>'}")
        print(f"         job_id={notebook_run_job_id or '<none>'}")

    if notebook_run_failed and cfg.notebook_run_enforce_success:
        print("[ERROR] Launcher notebook orchestration did not reach a verified success state.")
        print("       Set NOTEBOOK_RUN_ENFORCE_SUCCESS=false to keep legacy soft-fail behavior.")
        record_failure_event(
            stage="launcher_notebook_orchestration",
            code="NotebookOrchestrationFailed",
            message="Launcher notebook did not reach verified success state.",
            finalStatus=notebook_run_final_status,
            notebookName=cfg.notebook_name,
            jobId=notebook_run_job_id,
            workspaceId=ws_id,
            retries=cfg.notebook_run_max_attempts,
            enforceSuccess=cfg.notebook_run_enforce_success,
        )
        return 1

    if ws_id and cfg.automate_foundry_remaining:
        print("[STEP] Automating remaining Foundry/Fabric integration steps")
        max_attempts = max(1, int(os.getenv("FOUNDRY_REMAINING_MAX_ATTEMPTS", "2")))
        retry_delay_seconds = max(5, int(os.getenv("FOUNDRY_REMAINING_RETRY_DELAY_SECONDS", "30")))
        remaining_ok = False

        for attempt in range(1, max_attempts + 1):
            try:
                print(f"[INFO] Foundry remaining automation attempt {attempt}/{max_attempts}")
                run_with_wait_output(
                    ["python3", "scripts/automation/automate_foundry_remaining.py"],
                    label="Automating remaining Foundry steps",
                ...
                print(f"[WARN] Remaining Foundry automation step failed on attempt {attempt}: {ex}")
                if attempt < max_attempts:
                    print(f"[INFO] Retrying remaining Foundry automation in {retry_delay_seconds}s")
                    time.sleep(retry_delay_seconds)
                else:
                    print("       Manual fallback is documented in README.md under Foundry automation fallback steps.")
                    record_failure_event(
                        stage="foundry_remaining_automation",
                        code="FoundryRemainingAutomationFailed",
                        message=str(ex),
                        workspaceId=ws_id,
                        attempts=max_attempts,
                    )
                    maybe_generate_foundry_support_bundle()

        if not remaining_ok and os.getenv("FOUNDRY_AUTOMATION_ENFORCE_SUCCESS", "false").strip().lower() == "true":
            return 1
